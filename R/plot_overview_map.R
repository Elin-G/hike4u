#' Plot Overview Map
#'
#' `plot_overview_map` plots an overview map of the chosen route, by closeness value, and exports the map as a PNG file to the working directory.
#'
...
plot_overview_map <- function(longitude, latitude, final_routes, closeness_value) {
  ...
  basemaps::set_defaults(map_service = "esri", map_type = "world_street_map")
  ...
  if (intersect_df$intersect == 2) {
    ...
    p <- ggplot2::ggplot() +
      ...
      ggplot2::coord_sf() +
      ggplot2::geom_sf(data = closest_route, ggplot2::aes(color = "black"), linewidth = 1) +  # Hike in brown
      ggplot2::geom_sf(data = closest_route$start_point, ggplot2::aes(color = "red"), size = 6, shape = 18) +  # Starting Points in red
      ggplot2::geom_sf(data = poi, ggplot2::aes(color = "#333333"), size = 4, shape = 8) + # POI in black
      ggplot2::scale_color_manual(values = c("black" = "black", "red" = "red", "#333333" = "#333333"),
  ...
  ggplot2::ggsave(paste0("closest_route_nr_", closeness_value, "_overview_map.png"), plot = p, width = 12.5, height = 11, dpi = 300, bg = "white")
}
